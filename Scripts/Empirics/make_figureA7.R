...
#Set end for rectangles equal to 4 days
rdf <- mutate(rdf, end = start + 3600*24*4-1)
...
#For referee: calculate overlap between actual and these potential closures
{
  ...
  mean(closed_nc$intersectspotcl) #0.8658537
  ...
  # 0.0000  0.1334  0.4455  0.4482  0.6986  1.1000 
  
  summary(closed_nc$intersect_time_area_frac)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  # 0.00000 0.08249 0.33447 0.37034 0.59988 1.00000 
...
chmjuvsstart <- changejuv + chjuvsoutside #46473.31
...
finaldf <- rename(finaldf, juvcoef_4day = juvcoef, 
                  juvse_4day = juvse)

finaldf <- mutate(finaldf, 
                  juvcoef_4day_ub = juvcoef_4day + juvse_4day*qnorm(.975),
                  juvcoef_4day_lb = juvcoef_4day - juvse_4day*qnorm(.975))
...
paperFig <- function(myvar, ylab){
  ...
  ggsave(tbt, file=paste0("Output/Figures/figureA7.png"),
         w=7,h=(7/1.69)*2, units = "in", dpi=1200)
}


paperFig("juvcoef_4day", TeX("$\\beta_{st}$ coefficient and 95% confidence interval (Equation 1)"))
