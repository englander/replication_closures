...
#Now make each potential closure 40% bigger so they are same average size as actual closures
#Make a rectangular polygon from 4 points
#Compute horizontal length and 
biggerrect <- function(rowind){
  
  #Project
  poly <- st_transform(rdf[rowind,], st_crs("+proj=laea +lon_0=-76.5"))
  
  
  #Make length and height sqrt(1.4) longer
  polylength <- st_bbox(poly)["xmax"] - st_bbox(poly)["xmin"]
  polyheight <- st_bbox(poly)["ymax"] - st_bbox(poly)["ymin"]
  
  lt <- c(st_bbox(poly)["xmin"] - polylength*((sqrt(1.4)-1)/2),
          st_bbox(poly)["ymax"] + polyheight*((sqrt(1.4)-1)/2))
  rt <- c(st_bbox(poly)["xmax"] + polylength*((sqrt(1.4)-1)/2), 
          st_bbox(poly)["ymax"] + polyheight*((sqrt(1.4)-1)/2))
  rb <- c(st_bbox(poly)["xmax"] + polylength*((sqrt(1.4)-1)/2),
          st_bbox(poly)["ymin"] - polyheight*((sqrt(1.4)-1)/2))
  lb <- c(st_bbox(poly)["xmin"] - polylength*((sqrt(1.4)-1)/2),
          st_bbox(poly)["ymin"] - polyheight*((sqrt(1.4)-1)/2))
  ...
  poly <- rbind(lt, rt, rb, lb, lt) %>%
    list() %>%
    st_polygon() %>%
    st_sfc(crs = st_crs("+proj=laea +lon_0=-76.5")) %>% st_sf()
  
  names(poly)[length(poly)] <- "geometry"
  st_geometry(poly) <- "geometry"
  
  #Project back into lonlat
  poly <- st_transform(poly, st_crs(rdf))
  
  #Replace geometry
  out <- rdf[rowind,]
  out$geometry <- poly$geometry
  
  return(out)
}

#Apply over rows of rdf
rdflist <- lapply(1:nrow(rdf), function(x){
  biggerrect(x)
})

rdf <- do.call("rbind", rdflist)
...
finaldf <- rename(finaldf, juvcoef_bigger = juvcoef, 
                  juvse_bigger = juvse)

finaldf <- mutate(finaldf, 
                  juvcoef_bigger_ub = juvcoef_bigger + juvse_bigger*qnorm(.975),
                  juvcoef_bigger_lb = juvcoef_bigger - juvse_bigger*qnorm(.975))
