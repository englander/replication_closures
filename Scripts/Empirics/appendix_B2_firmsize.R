...
#Join these categories onto fullbe
fullbe <- left_join(fullbe, 
                    dplyr::select(owndf, Matricula, Temporada, fleettype), 
...
#Given row of rddf, BE data filtered to same time period, and fleet type,
#calculate outcomes
outFleet <- function(row, mybesf, myfleet){
  
  #Filter mybesf to given fleet type
  fleetrows <- filter(mybesf, fleettype == myfleet)
  ...
  out <- data.frame(fleettype = myfleet, tons=tons,
...
outcomesFun <- function(rdrow){
  ...
  if(nrow(mybesf)>0){
    ...
    #Apply over fleettypes
    out <- map_df(unique(besf$fleettype), function(x){
      outFleet(row, mybesf, x)
    }) %>% 
      #Add bin and rid
    ...
    out <- data.frame(bin=row$bin, rid=row$rid,
                      fleettype = unique(besf$fleettype),
...
heterodf$fleettype <- as.factor(heterodf$fleettype)
...
juvcatch <- felm(
  as.Formula(paste0(
    "asinhnummjuv", "~ ", 
    paste0(
      paste0(grep("_treatfrac",names(heterodf),value=T), ":fleettype"),
      collapse="+"),
    ...
    "| bin:fleettype + twowk:cellid_2p + startdate",
    " | 0 | twoweek_cellid_2p")),
  data = heterodf)
...
jvtab$fleettype <- "singleton"
jvtab$fleettype[grep("fleettypelarge",jvtab$bin)] <- "large"
jvtab$fleettype[grep("fleettypemedium",jvtab$bin)] <- "medium"

jvtab$bin <- gsub("fleettype","",jvtab$bin)
jvtab$bin <- gsub("singleton","",jvtab$bin)
jvtab$bin <- gsub("large","",jvtab$bin)
jvtab$bin <- gsub("medium","",jvtab$bin)
...
tonscoef <- as.data.frame(tonscoef) %>% 
  mutate(fleettype = gsub("treatfrac:fleettype","",rownames(tonscoef)))
...
effectLength <- function(myfleet){
  ...
  tons1 <- sum(fullbe$betons[fullbe$Temporada!="2017-II" & fullbe$Temporada!="2019-II" & 
                               fullbe$fleettype==myfleet])
  
  #Change in tons because of policy
  ctons <- tons1 - tons1/exp(tonscoef$Estimate[tonscoef$fleettype==myfleet]) 
  ...
  avgpjoutside <- filter(fullbe, lead_0==0 & lead_10==0 & lead_20==0 & lead_30==0 & lead_40==0 & lead_50==0 & 
                           ...
                           Temporada!="2017-II" & Temporada!="2019-II" & fleettype==myfleet) %>%
    #Weight by tons
  ...
  juv1 <- sum(fullbe$numjuv[fullbe$fleettype==myfleet], na.rm=T) / 10^6
  ...
  out <- data.frame(fleettype=myfleet, chmjuvsstart = chmjuvsstart, totper = totper)
